...
Only the portion of [ReactiveMongo-Extensions](https://github.com/ReactiveMongo/ReactiveMongo-Extensions) _originally from this project_ is now being extracted from the now-defunct extensions project.  ReactiveMongo-Criteria should remain API compatible with all versions of [ReactiveMongo](https://github.com/ReactiveMongo/ReactiveMongo) as this library only depends on the public API from the driver.
...
- Ability to ''type check'' query constraints by specifying a Scala type. *FIRST LEVEL PROPERTIES COMPLETE*
